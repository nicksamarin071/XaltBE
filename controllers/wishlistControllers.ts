...
export const addwishlistController = async (req:Request, res: Response): Promise<any> => {
    try {
...
export  const getUserWishlistController = async (req:Request, res: Response): Promise<any> => {
    try {
    const user_id = req.session?._id; 
    if (!user_id) {
       return resSend(res, 401, "", null);
...
export const removeWishlistController = async(req: Request, res: Response): Promise<any> =>{
try {
  const user_id = req.session?._id; 
  if (!user_id) {
       return resSend(res, 401, "", null);
